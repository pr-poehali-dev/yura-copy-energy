...
export default function Index() {
  ...
  const handleSend = async () => {
    ...
    const smartResponses: Record<string, string> = {
      'создай': '🚀 Отлично! Я готов создать для тебя ',
      'сделай': '⚡ Начинаю работу! ',
      'исправ': '🔧 Сейчас найду и исправлю! ',
      'добав': '✨ Добавляю ',
      'помоги': '💫 Конечно помогу! ',
      'что ты': '🤖 Я Юра — твой личный разработчик! Могу создавать сайты, писать код, исправлять баги и деплоить проекты.',
      'привет': '👋 Привет! Готов приступить к работе! Энергия на максимуме!',
      'спасибо': '🙌 Всегда пожалуйста! Обращайся, если нужна помощь!',
      'как дела': '⚡ Дела отлично! Безлимитная энергия позволяет работать 24/7!'
    };
    ...
    for (const [keyword, response] of Object.entries(smartResponses)) {
      if (lowerMessage.includes(keyword)) {
        ...
        if (keyword === 'создай' || keyword === 'сделай' || keyword === 'добав') {
          responseText += userMessage.slice(keyword.length).trim() + '! Скажи "начинай" и я приступлю 🚀';
        }
        break;
      }
    }
    ...
    if (!responseText) {
      const defaultResponses = [
        ...
        'Готово к выполнению! Скажи "начинай" 🚀',
        'Обрабатываю запрос... Производительность 999%! ⚡',
        'Классная задача! Я знаю как это сделать 💪',
        'Безлимитная энергия активирована! Приступаю 🌟'
      ];
